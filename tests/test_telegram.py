...
_PERFIL_EXEMPLO = "dados_bi"

CASOS_PARSE_CALLBACK = [
    (
        "positivo-formato-valido",
        f"fb|1|{_PERFIL_EXEMPLO}|{_ID_EXEMPLO}",
        (_PERFIL_EXEMPLO, _ID_EXEMPLO, "positivo"),
    ),
    (
        "negativo-formato-valido",
        f"fb|0|cx|{_ID_EXEMPLO}",
        ("cx", _ID_EXEMPLO, "negativo"),
    ),
    # Botões enviados antes da centralização continuam funcionando.
    ("formato-legado", f"fb|1|{_ID_EXEMPLO}", (None, _ID_EXEMPLO, "positivo")),
    ...
    # Formato sem as 3/4 partes esperadas.
    ("sem_partes_suficientes", "fb|1", None),
    ("partes_demais", f"fb|1|cx|{_ID_EXEMPLO}|extra", None),
    ("string_vazia", "", None),
]
...
def test_teclado_feedback_estrutura():
    teclado = _teclado_feedback(_ID_EXEMPLO, _PERFIL_EXEMPLO)
    linha = teclado["inline_keyboard"][0]
    assert len(linha) == 2
    assert linha[0]["callback_data"] == f"fb|1|{_PERFIL_EXEMPLO}|{_ID_EXEMPLO}"
    assert linha[1]["callback_data"] == f"fb|0|{_PERFIL_EXEMPLO}|{_ID_EXEMPLO}"
...
def test_ida_e_volta_teclado_para_parser():
    ...
    assert _parsear_callback_data(botao_positivo["callback_data"]) == (
        _PERFIL_EXEMPLO, _ID_EXEMPLO, "positivo"
    )
    assert _parsear_callback_data(botao_negativo["callback_data"]) == (
        _PERFIL_EXEMPLO, _ID_EXEMPLO, "negativo"
    )
